...
export default function CreateMaterialPage() {
  ...
  const handleChange = (field: keyof CreateMaterialData, value: string | boolean | undefined) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = "نام متریال الزامی است";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(`متریال ${formData.name} با موفقیت ایجاد شد`);
      router.push("/dashboard/admin/materials");
    } catch (error: unknown) {
      logger.error("Error creating material", error);
      const errorMessage = (error as any)?.response?.data?.message || "خطا در ایجاد متریال";
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-brand-medium-blue">
            اطلاعات متریال جدید را وارد کنید
          </p>
        </div>
