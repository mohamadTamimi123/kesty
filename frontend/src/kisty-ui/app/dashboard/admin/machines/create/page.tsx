...
import { ArrowRightIcon, DocumentTextIcon, WrenchScrewdriverIcon } from "@heroicons/react/24/outline";
...
export default function CreateMachinePage() {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof CreateMachineData, string>>>({});
  ...
  const handleChange = (field: keyof CreateMachineData, value: string | boolean | undefined) => {
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof CreateMachineData, string>> = {};

    if (!formData.name.trim()) {
      newErrors.name = "نام دستگاه الزامی است";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(`دستگاه ${formData.name} با موفقیت ایجاد شد`);
      router.push("/dashboard/admin/machines");
    } catch (error: unknown) {
      logger.error("Error creating machine", error);
      const errorMessage = (error as any)?.response?.data?.message || "خطا در ایجاد دستگاه";
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-brand-medium-blue">
            اطلاعات دستگاه جدید را وارد کنید
          </p>
        </div>
